...
embedding_model = SentenceTransformer('/home/tzl/Projects/LLM/stage2/Labse', local_files_only=True).to(device)
...
def generate_guideline_diffs(type1, type2, similar_samples, texts, llm):
    ...
    response, din1, dout1 = llm.chat(str(prompt_1), None, 64)
    guideline_1 = extract_complete_sentences(text=str(response), max_words=64)
    # type2
    prompt_2 = prompt + f'\nAccording to these confusing examples, compared to the "{type1}" type, the "{type2}" refers to: '
    response, din2, dout2 = llm.chat(str(prompt_2), None, 64)
    guideline_2 = extract_complete_sentences(text=str(response), max_words=64)
    input_tokens = input_tokens + din1 + din2
    output_tokens = output_tokens + dout1 + dout2
        ...
        for i in tqdm(range(len(labels))):
            for j in range(i+1, len(labels)):
                ...
                if len(similar_samples) > 29:
                    tmp_similar_samples = []
                    for obj in similar_samples:
                        if (len(obj[type1]) > 7) and (len(obj[type2]) > 7):
